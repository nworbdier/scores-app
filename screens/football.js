...
const FOOTBALL = ({ route }) => {
  ...
  const fetchDates = async () => {
    try {
      const response = await fetch(
        `https://sports.core.api.espn.com/v2/sports/football/leagues/${sport.toLowerCase()}/calendar/whitelist?dates=2024`
      );
  ...
  const fetchGameData = async () => {
    try {
      ...
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/football/${sport.toLowerCase()}/scoreboard?dates=${formattedDate}`
      );
